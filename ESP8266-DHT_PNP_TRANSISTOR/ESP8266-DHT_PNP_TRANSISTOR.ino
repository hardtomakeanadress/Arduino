...
DHT dht(DHTPIN, DHT11,15); // 11 works fine for ESP8266
...
void gettemperature() {
  ...
  //remember, this is for driving a PNP transitor;  relay is ON when transitor is LOW :)
  if (temp_f <= heatOn)
  {
...
void setHTML()
{
  ...
  webString += "      min: 0, max: 60,";
  webString += "      greenFrom: 18, greenTo: " + String(heatOn) + ",";
  webString += "      yellowFrom: " + String(heatOn) + ", yellowTo: " + String(heatOff) + ",";
  webString += "      redFrom: " + String(heatOff) + ", redTo: 60,";
  ...
  webString += "      greenFrom: 30, greenTo: 60,";
  webString += "      yellowFrom: 60, yellowTo: 75,";
  ...
  webString += "         0: {title: 'Temp Celsius'}, \n";
  ...
  webString += "<h1>ESP8266 DHT Thermostat IoT</h1>\n";
  webString += "<div style=\"color:red\">" + webMessage + "</div>\n";

  webString += "<table style=\"width:800px;\"><tr><td>";
  webString += "<div>Heat On Setting: &le; " + String((int)heatOn) + "&deg; C</div>\n";
  webString += "<div>Heat Off Setting: &ge; " + String((int)heatOff) + "&deg; C</div>\n";
  ...
  webString += "<div>Temperature: " + String(temp_f, 2) + "&deg; C</div>\n";
...
void setup(void)
{
  ...
  Serial.begin(115200);  // Serial connection from ESP-01 via 3.3v console cable
                         //REMEMBER, THIS IS A PNP TRANSISTOR: WHEN PIN IS LOW TRANSISTOR IS ON :)
